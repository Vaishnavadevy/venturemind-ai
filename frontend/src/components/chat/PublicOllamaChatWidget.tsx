...
export function PublicOllamaChatWidget() {
  ...
  const ask = async () => {
    ...
    setQuestion('')
    try {
      const { data } = await apiClient.post<APIResponse<Reply>>('/public-advisor', {
        question: trimmed,
        history: messages.slice(-8),
      }, { timeout: 130_000 })
      setMessages((current) => [...current, { role: 'assistant', content: data.data.response }])
    } catch (caughtError) {
  ...
  return <div className="fixed bottom-5 right-5 z-50">
    {open && <section className="mb-3 w-[min(24rem,calc(100vw-2.5rem))] overflow-hidden rounded-3xl border border-violet-200 bg-white shadow-2xl shadow-slate-950/20">
      ...
      <div className="max-h-72 space-y-3 overflow-y-auto p-4">{messages.length > 0 ? messages.map((message, index) => <article key={`${message.role}-${index}`} className={`max-w-[92%] whitespace-pre-wrap rounded-2xl p-3 text-sm leading-6 ${message.role === 'user' ? 'ml-auto bg-violet-600 text-white' : 'bg-violet-50 text-slate-800'}`}><p className={`mb-1 text-[0.65rem] font-bold uppercase tracking-wider ${message.role === 'user' ? 'text-violet-100' : 'text-violet-700'}`}>{message.role === 'user' ? 'You' : 'VentureMind'}</p>{message.content}</article>) : <p className="rounded-2xl bg-slate-50 p-4 text-sm leading-6 text-slate-600">Ask about validating an idea, pricing, finding customers, MVP scope, or preparing to launch.</p>}{thinking && <p className="rounded-2xl bg-slate-100 p-3 text-sm text-slate-600">VentureMind is thinking…</p>}{error && <p className="rounded-xl bg-rose-50 p-3 text-sm text-rose-700">{error}</p>}<div ref={conversationEndRef} /></div>
